...
class Categorization(object):
    ...
    def __opp_normalization(self, signatures, dataset_name):
        """
    ...
    def __get_signatures_from_best_nmf_model(self, dataset_name, k, reps=10):
        ...
        for i in np.arange(reps):
            model = NMF(n_components=k, init='random', max_iter=2000, solver='mu',
        ...
        signatures = self.__opp_normalization(signatures, dataset_name)
    ...
    def __generate_categories(self, n_generate):
        ...
        while len(generated_categories) != n_generate:
            ...
            loc_start_max = min(sub_loc, KMER_SIZE - n_of_bases)
    ...
    def __get_normalizers(self):
        """
        Compute mutation opportunity normalizers for each dataset, according to the input WGS/WXS sequence count data
    ...
    def __get_catalogs(self):
        """
        Compute normalized WGS and WXS mutation catalogs (n samples x m categories) for each dataset, according to the
        ...
        for dataset in DATASETS:
            ...
            for k in dataset.k_range:
                ...
                for repeat in np.arange(REPEATS):
                    folds = dataset.cca_folds['train'][repeat]
    ...
    def mutate(self):
        ...
        for idx in categories_to_mutate:
            ...
            rel_for_mutation_idx = [idx for idx in np.arange(len(new_cat.sequence)) if
                                    idx + new_cat.start_idx not in [KMER_SIZE // 2 - 1, KMER_SIZE // 2]]
...
class Dataset(object):
    def __init__(self, path_to_wgs_mutation_data=None, path_to_mutation_data=None, path_to_ge_data=None,
        ...
        for repeat in np.arange(REPEATS):
            ...
            self.cca_folds['train'][repeat] = np.array_split(indices_train, N_FOLDS)
            indices_test = np.random.permutation(self.test_samples_idx)
            self.cca_folds['test'][repeat] = np.array_split(indices_test, N_FOLDS)
...
def single_crossover(parents):
    ...
    if not crossover_flag:  # only mutate fit categorization
        ...
        if len(categories_not_in_fit_parent) >= take_from_weak:
            ...
            n_to_add = take_from_weak - len(categories_not_in_fit_parent)
            print("NOT ENOUGH UNIQUE CATEGORIES FOR CROSSOVER --> CREATING " + str(n_to_add) + " NEW ONES")
            categories_from_fit_parent = np.random.choice(s_strong_parent, size=take_from_strong, replace=False)
...
if __name__ == "__main__":
    ...
    # general parameters
    parser.add_argument('--datatype', type=str, default='brca', help='datasets separated with _')
    parser.add_argument('--opt_k', type=str, default='', help='optimal Ks separated with _')
    parser.add_argument('--kmer', type=int, default=8, choices=[8], help='8mer mutation sequences')
    ...
    DDR_GENE_SET = np.load('DDR_genes.npy', allow_pickle=True)
    WGS_OPPORTUNITY_FILE = 'wgs_opportunity_7mer_dictionary.pickle'
    WXS_OPPORTUNITY_FILE = 'wxs_opportunity_7mer_dictionary.pickle'
    ...
    EMPTY_CATEGORY = Category(start_idx=0, sequence='N' * KMER_SIZE)
    DATASETS = []
    DATASET_NAMES = args.datatype.split("_")
    OPT_K = args.opt_k.split("_")

    for i, dataset_name_glb in enumerate(DATASET_NAMES):
        DATASETS.append(Dataset(path_to_wgs_mutation_data='categorization_related_data/%s_wgs_catalog.csv'
                                                          % dataset_name_glb,
                      path_to_mutation_data='categorization_related_data/%s_catalog.csv' % dataset_name_glb,
                      path_to_ge_data='categorization_related_data/%s_ge.csv' % dataset_name_glb,
                                dataset_name=dataset_name_glb, optimal_k=int(OPT_K[i]), k_range=None))
    timestamp = datetime.datetime.now().strftime('%d-%m_%H-%M-%S')
    run_name = '%s_kmer-%d_mut-%.2f_q-%.2f_c-%d_power-%d_%s' % (args.datatype, KMER_SIZE, MUTATION_RATE, Q_CROSSOVER,
